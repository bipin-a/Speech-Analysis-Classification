...
figure
plot(tf,speech)
title('Female "U" Speech Time Domain')
xlabel('Time (s)')
ylabel('Amplitude')
...
title('Female "U" Autocorrelation Pitch Detection')
xlabel('Time Lag (s)')
...
title('Speech Spectrum of Female "U" Voice');
axis tight; 
% Finding all peaks in signal
Select_FFT = speech_FFT(1:M/2);
[f_pks, f_locs] = findpeaks(Select_FFT,ff_hz(1:M/2),'MinPeakDistance',70,'MinPeakProminence',10);
hold on
plot(f_locs,f_pks,'o')
% Fitting a polynomial line with order = 8 to the minimums of line
p = polyfit(f_locs, -f_pks',5);     % f_pks is negative to find peaks
v = polyval(p, f_locs);
plot(f_locs,v)                    %Plot polynomial commented out
...
% plot (seg_locs,seg_pks,'o')       %Plot polynomial commented out
...
for n = 1:length(formants_pks(1,:))
    if (n==1)
        rectangle('Position', [0 0 seg_locs(1,n) max(formants_pks(:,n))+50], ...
    ...
    elseif (n == length(formants_pks(1,:)))
        rectangle ('Position', [formants_locs(find(formants_locs(:,n),1),n)-100 0 ...
            ...
            max(formants_pks(:,n))+50], 'EdgeColor', 'b', 'LineWidth',2, 'LineStyle','-.')
    else
...
figure
plot(tf,speech)
title('Male "U" Speech Time Domain')
xlabel('Time (s)')
ylabel('Amplitude')
...
[pks, locs] = findpeaks(ACF_speech,tf,'MinPeakDistance',0.003,'NPeaks', NUMBER_PEAKS);
findpeaks(ACF_speech,tf,'MinPeakDistance',0.003,'SortStr','descend','NPeaks', NUMBER_PEAKS);
title('Male Autocorrelation "U" Pitch Detection')
xlabel('Time Lag (s)')
ylabel('Coefficiant of Correlation')
...
xlabel('Frequency (Hz)');
ylabel('Magnitude Spectrum');
title('Speech Spectrum of Male "U" Voice');
axis tight; 
Select_FFT = speech_FFT(1:M/2);
% Finding all peaks in signal
[f_pks, f_locs] = findpeaks(Select_FFT,ff_hz(1:M/2),'MinPeakDistance',80,'MinPeakProminence',4.5);
hold on
plot(f_locs,f_pks,'o')
% Fitting a polynomial line with order = 10 to the minimums of line
p = polyfit(f_locs, -f_pks',9);     % f_pks is negative to find peaks
v = polyval(p, f_locs);
plot(f_locs,v)                    % Plotting Polynomial commented out
% Finding x-axis boundaries of formants by finding peaks of Polynomial Line
[seg_pks, seg_locs] = findpeaks(v,f_locs,'MinPeakDistance',200);
% plot (seg_locs,seg_pks,'o')       % Plotting Polynomial commented out
