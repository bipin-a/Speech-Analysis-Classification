...
figure
plot(tf,speech)
title('Female "I" Speech Time Domain')
xlabel('Time (s)')
ylabel('Amplitude')
...
title('Female "I" Autocorrelation Pitch Detection')
xlabel('Time Lag (s)')
...
title('Speech Spectrum of Female "I" Voice');
axis tight; 
% Finding all peaks in signal
Select_FFT = speech_FFT(1:M/2);
[f_pks, f_locs] = findpeaks(Select_FFT,ff_hz(1:M/2),'MinPeakDistance',80,'MinPeakProminence',25);
hold on
plot(f_locs,f_pks,'o')
% Fitting a polynomial line with order = 7 to the minimums of line
p = polyfit(f_locs, -f_pks',6);     % f_pks is negative to find peaks
v = polyval(p, f_locs);
...
figure
plot(tf,speech)
title('Male "I" Speech Time Domain')
xlabel('Time (s)')
ylabel('Amplitude')
...
[pks, locs] = findpeaks(ACF_speech,tf,'MinPeakDistance',0.003,'NPeaks', NUMBER_PEAKS);
findpeaks(ACF_speech,tf,'MinPeakDistance',0.003,'SortStr','descend','NPeaks', NUMBER_PEAKS);
title('Male Autocorrelation "I" Pitch Detection')
xlabel('Time Lag (s)')
ylabel('Coefficiant of Correlation')
...
xlabel('Frequency (Hz)');
ylabel('Magnitude Spectrum');
title('Speech Spectrum of Male "I" Voice');
axis tight; 
Select_FFT = speech_FFT(1:M/2);
% Finding all peaks in signal
[f_pks, f_locs] = findpeaks(Select_FFT,ff_hz(1:M/2),'MinPeakDistance',20,'MinPeakProminence',20);
hold on
plot(f_locs,f_pks,'o')
% Fitting a polynomial line with order = 10 to the minimums of line
p = polyfit(f_locs, -f_pks',8);     % f_pks is negative to find peaks
v = polyval(p, f_locs);
