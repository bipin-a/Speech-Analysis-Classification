...
figure
plot(tf,speech)
title('Female "A" Speech Time Domain')
xlabel('Time (s)')
ylabel('Amplitude')
...
title('Female "A" Autocorrelation Pitch Detection')
xlabel('Time Lag (s)')
...
title('Speech Spectrum of Female "A" Voice');
axis tight; 
% Finding all peaks in signal
Select_FFT = speech_FFT(1:M/2);
[f_pks, f_locs] = findpeaks(Select_FFT,ff_hz(1:M/2),'MinPeakDistance',50,'MinPeakProminence',13);
hold on
plot(f_locs,f_pks,'o')
...
figure
plot(tf,speech)
title('Male "A" Speech Time Domain')
xlabel('Time (s)')
ylabel('Amplitude')
...
[pks, locs] = findpeaks(ACF_speech,tf,'MinPeakDistance',0.003,'NPeaks', NUMBER_PEAKS);
findpeaks(ACF_speech,tf,'MinPeakDistance',0.003,'SortStr','descend','NPeaks', NUMBER_PEAKS);
title('Male Autocorrelation "A" Pitch Detection')
xlabel('Time Lag (s)')
ylabel('Coefficiant of Correlation')
...
xlabel('Frequency (Hz)');
ylabel('Magnitude Spectrum');
title('Speech Spectrum of Male "A" Voice');
axis tight; 
Select_FFT = speech_FFT(1:M/2);
% Finding all peaks in signal
[f_pks, f_locs] = findpeaks(Select_FFT,ff_hz(1:M/2),'MinPeakDistance',50,'MinPeakProminence',4.5);
hold on
plot(f_locs,f_pks,'o')
